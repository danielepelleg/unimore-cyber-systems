...
$$
In questo modo *la complessità non è più infinita*. La **complessità** viene ridotta al ***numero di deadline in un iperperiodo***.
...
La mutua esclusione viene gestita mediante l'utilizzo di *semafori* sui quali vengono fatte operazioni di *lock* e *unlock*.
...
In generale ci si aspetta che il *tempo di bloccaggio massimo* del task ad alto priorità corrisponda alla *lunghezza della sezione critica del task* $\tau_2$, ma in realtà il *ritardo dovuto al fenomeno del bloccaggio può essere anche peggiore*. 
...
Il fatto che un task a più alta priorità esegua al posto di uno a priorità più bassa non è un problema, diventa un problema quando accade il contrario: quando un task a *priorità più bassa* esegue al posto di uno a *priorità più alta*. La situazione precedente può complicarsi ulteriormente anticipando l'arrivo del task $\tau_2$.

<img src="resources/03_21_conflitto_sezione_critica_2.PNG" style="zoom:30%;" />
