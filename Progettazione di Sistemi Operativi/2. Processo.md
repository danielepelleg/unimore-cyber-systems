...
Il problema base della competizione è quello della ***mutua esclusione*** cioè la *competizione per l’uso di risorse comuni che non possono essere usate contemporaneamente*.

I *processi concorrenti* interagenti che compongono un processo non  sequenziale introducono dei *vincoli di sincronizzazione tra gli eventi* dei processi concorrenti.
...
- *invio e ricezione di messaggi* (con trasferimento di dati) &rarr;  in questo caso la *sincronizzazione che è necessaria* fra i processi spesso viene detta `COMUNICAZIONE`
...
Si tratta di un'interazione *non prevedibile e non desiderata* che dipende dalla ***velocità relativa*** tra i processi.

### Sincronizzazione

La sincronizzazione rappresenta un ***vincolo sull’ordine con cui sono eseguite le operazioni*** sui processi.

- Nel caso di INTERAZIONE INDIRETTA, il vincolo deve rendere IMPOSSIBILE l’esecuzione contemporanea delle operazioni con le quali i processi accedono a risorse comuni
...
In ogni caso sono necessari **MECCANISMI DI SINCRONIZZAZIONE**

Non usare sincronizzazione o sbagliarla implica introdurre degli ERRORI DIPENDENTI DAL TEMPO

 POSSIBILITÀ di CORSE CRITICHE (RACE CONDITION)

In particolare, IN CASO DI INTERAZIONE INDIRETTA  possibilità che per certi rapporti di velocità di esecuzione dei processi interagenti, le operazioni su risorse comuni generino interferenze e quindi errori